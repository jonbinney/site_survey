...
#*CAPABILITY: PACKET type,encryption,channel,datarate,ssid,bssid,sourcmac,destmac,signal_dbm,signal_rssi,noise_dbm,noise_rssi
  if int(fields['signal_dbm']) > -40:
    if fields['sourcmac'] != '00:00:00:00:00:00':
      if fields['bssid'] == fields['sourcmac']:
        print "Got packet from AP %s with signal strength %s on channel %s"%(
            fields['bssid'], fields['signal_dbm'], fields['channel'])
      else:
        print "Packet from %s with signal %s on channel %s"%(fields['sourcmac'],
            fields['signal_dbm'], fields['channel'])
    elif fields['destmac'] != '00:00:00:00:00:00':
